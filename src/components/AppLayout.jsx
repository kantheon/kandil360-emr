  ...
  return (
    <div className="min-h-screen flex bg-[#f0f4f8]">
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 h-screen bg-white border-r border-border-light flex flex-col transition-all duration-300 ease-in-out ${
        ...
        {/* Heart toggle */}
        <div className={`flex items-center h-14 border-b border-border-light shrink-0 ${collapsed ? 'justify-center px-0' : 'gap-3 px-4'}`}>
          <button
            onClick={() => {
          ...
          {!collapsed && (
            <>
              <div className="overflow-hidden">
                <h1 className="text-sm font-bold text-text-primary leading-tight whitespace-nowrap">Kandil360</h1>
                <p className="text-[10px] text-text-muted font-medium whitespace-nowrap">Case Management EMR</p>
              </div>
          ...
          {navItems.map(({ to, icon: Icon, label, end }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ...
                } ${isActive ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-text-secondary hover:bg-surface-hover hover:text-text-primary'}`
              }
            >
        ...
        <div className={`border-t border-border-light shrink-0 ${collapsed ? 'p-2' : 'p-3'}`}>
          <div className="relative">
            <button
              ...
              className={`w-full flex items-center gap-3 rounded-xl transition-colors cursor-pointer hover:bg-surface-hover ${collapsed ? 'justify-center p-1.5' : 'px-3 py-2'}`}
            >
              <img
            ...
            {userMenuOpen && (
              <>
                ...
                <div className={`absolute z-20 bg-white rounded-xl shadow-lg border border-border-light p-1 animate-fade-in ${collapsed ? 'left-full bottom-0 ml-2 w-48' : 'bottom-full left-0 mb-1 w-full'}`}>
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium text-danger-600 hover:bg-danger-50 transition-colors cursor-pointer mt-1"
                  >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="lg:hidden sticky top-0 z-30 h-11 bg-white border-b border-border-light flex items-center px-3 shrink-0">
          ...
          <div className="flex items-center gap-2 ml-2">
            ...
            <span className="text-sm font-bold text-text-primary">Kandil360</span>
          </div>
        </div>
